...
const TrackPackages = () => {
    ...
    const handleSubscribe = async () => {
        try {
            ...
            // Find current user's package by userId
            const existingPackage = packages.find(pkg => pkg.userId === user?.email);

            if (existingPackage && existingPackage.packageType) {
                // User already has a package
                Swal.fire({
                    title: 'Subscription Error',
                    text: 'You already have an active package subscription. Please manage your existing package first.',
                    icon: 'error',
            ...
            packageData.userId = user?.userId;

            // Build URL-encoded query params
            ...
            if (!response.ok) throw new Error("Package subscription failed");

            const result = await response.json();

            Swal.fire({
                title: 'Success!',
                text: 'Your package subscription was successful!',
                icon: 'success',
